...
- (void)setUploadProgressDelegate:(id)newDelegate {
	...
	[self resetProgressDelegate: &uploadProgressDelegateLocal];
...
- (void)setDownloadProgressDelegate:(id)newDelegate {
	...
	[self resetProgressDelegate:&downloadProgressDelegateLocal];
}
...
- (void)resetProgressDelegate:(id*)progressDelegate {
	...
	if ([*progressDelegate respondsToSelector:selector]) {
		float value = 0.0f;
        //id progressDelegateLocal = (__strong*)progressDelegate;
		[ASIHTTPRequest performSelector:selector onTarget: progressDelegate withObject:nil amount:&value callerToRetain:nil];
	}
#endif
}
...
- (void)request:(ASIHTTPRequest *)request didReceiveBytes:(long long)bytes {
	...
	if ([self downloadProgressDelegate]) {
        id downloadProgressDelegateLocal = downloadProgressDelegate;
		[ASIHTTPRequest updateProgressIndicator:&downloadProgressDelegateLocal withProgress:[self bytesDownloadedSoFar] ofTotal:[self totalBytesToDownload]];
	}
}
...
- (void)request:(ASIHTTPRequest *)request didSendBytes:(long long)bytes {
	...
	if ([self uploadProgressDelegate]) {
        id uploadProgressDelegateLocal = uploadProgressDelegate;
		[ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegateLocal withProgress:[self bytesUploadedSoFar] ofTotal:[self totalBytesToUpload]];
	}
}
