...
- (void)startRequest
{
...
#if TARGET_OS_IPHONE
			NSDictionary *proxySettings = CFBridgingRelease((__bridge CFTypeRef)((__bridge NSDictionary *)CFNetworkCopySystemProxySettings()));
#else
			NSDictionary *proxySettings = NSMakeCollectable([(NSDictionary *)SCDynamicStoreCopyProxies(NULL) autorelease]);
#endif
			
			proxies = CFBridgingRelease((__bridge CFTypeRef)((__bridge NSArray *)CFNetworkCopyProxiesForURL((__bridge CFURLRef)[self url], (__bridge CFDictionaryRef)proxySettings)));
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([[self connectionInfo] objectForKey:@"stream"]) {
			oldStream = [[[self connectionInfo] objectForKey:@"stream"] copy];
...
- (void)checkRequestStatus
{
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			[self setTotalBytesSent:[CFBridgingRelease((__bridge CFTypeRef)((NSNumber *)CFBridgingRelease(CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount)))) unsignedLongLongValue]];
...
- (void)updateDownloadProgress
{
    ...
    id queueLocal = queue;
    id downloadProgressDelegateLocal = downloadProgressDelegate;
	[ASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queueLocal withObject:self amount:&value callerToRetain:self];
	[ASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegateLocal withObject:self amount:&value callerToRetain:self];

	[ASIHTTPRequest updateProgressIndicator:&downloadProgressDelegateLocal withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
    ...
    id queueLocal = queue;
    id uploadProgressDelegateLocal = uploadProgressDelegate;
	[ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queueLocal withObject:self amount:&value callerToRetain:self];
	[ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegateLocal withObject:self amount:&value callerToRetain:self];
	[ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegateLocal withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
- (void)incrementDownloadSizeBy:(long long)length
{
    id queueLocal = queue;
    id downloadProgressDelegateLocal = downloadProgressDelegate;
	[ASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queueLocal withObject:self amount:&length callerToRetain:self];
	[ASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegateLocal withObject:self amount:&length callerToRetain:self];
...
- (void)incrementUploadSizeBy:(long long)length
{
    id queueLocal = queue;
    id uploadProgressDelegateLocal = uploadProgressDelegate;
	[ASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queueLocal withObject:self amount:&length callerToRetain:self];
	[ASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegateLocal withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queueLocal withObject:self amount:&progressToRemove callerToRetain:self];
	[ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegateLocal withObject:self amount:&progressToRemove callerToRetain:self];
	[ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegateLocal withProgress:0 ofTotal:[self postLength]];
...
- (void)readResponseHeaders
{
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		NSString *httpVersion = CFBridgingRelease((__bridge CFTypeRef)((__bridge NSString *)CFHTTPMessageCopyVersion(message)));
...
- (void)markAsFinished
{
	...
	if (request) {
		CFBridgingRelease(request);
	}
	if (requestAuthentication) {
		CFBridgingRelease(requestAuthentication);
	}
	if (proxyAuthentication) {
		CFBridgingRelease(proxyAuthentication);
	}
...
{
	NSError * underlyingError = CFBridgingRelease(CFReadStreamCopyError((CFReadStreamRef)[self readStream]));
...
+ (void)setSessionCookies:(NSMutableArray *)newSessionCookies
{
    ...
    sessionCookies = nil;
	sessionCookies = [newSessionCookies copy];
...
+ (NSArray *)proxiesForURL:(NSURL *)theURL fromPAC:(NSURL *)pacScriptURL
{
	...
	NSArray *proxies = CFBridgingRelease(CFNetworkCopyProxiesForAutoConfigurationScript((__bridge CFStringRef)script,(__bridge CFURLRef)theURL, &err2));
	if (err2) {
		return nil;
	}
	return proxies;
...
+ (NSString *)mimeTypeForFileAtPath:(NSString *)path
{
    ...
    return CFBridgingRelease((__bridge CFTypeRef)((__bridge NSString *)MIMEType));
}
...
+ (void)recordBandwidthUsage
{
    ...
    bandwidthMeasurementDate = nil;
	bandwidthMeasurementDate = [[NSDate dateWithTimeIntervalSinceNow:1] copy];
